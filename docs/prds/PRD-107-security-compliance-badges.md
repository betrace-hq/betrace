...
**Critical for BeTrace:** Handling sensitive telemetry data requires strong security messaging.

## Problem Statement

Without security/compliance badges, prospects:
1. Question BeTrace's security posture ("Can I trust this with production traces?")
...
**Description:**
```markdown
BeTrace's security architecture has been audited by independent security experts.
...
**Description:**
```markdown
All compliance evidence generated by BeTrace is cryptographically signed.
...
**Description:**
```markdown
BeTrace's rule engine uses capability-based security to prevent malicious rules.
...
**Description:**
```markdown
BeTrace detects and redacts PII before exporting to OpenTelemetry collectors.
...
**Description:**
```markdown
BeTrace implements SOC2 Trust Service Criteria controls.
...
**Description:**
```markdown
BeTrace supports HIPAA Technical Safeguards for protected health information (PHI).
...
**Description:**
```markdown
BeTrace aligns with ISO27001 information security controls.
...
**Description:**
```markdown
BeTrace maintains high test coverage standards.
...
**Description:**
```markdown
BeTrace uses Nix flakes for reproducible, secure builds.
...
**Description:**
```markdown
BeTrace's architecture, PRDs, and security models are openly documented.
...
**Description:**
```markdown
BeTrace is a pure application framework with no infrastructure lock-in.
...
- **Colors:** Match BeTrace brand (blues, grays)
...
**Critical Principle:** Never claim certification BeTrace doesn't have.
...
**Description:**
```markdown
BeTrace is NOT currently certified for any compliance framework.
...
3. Write FAQ section ("Is BeTrace SOC2 certified?")
...
### Risk: Competitors claim certifications BeTrace doesn't have
