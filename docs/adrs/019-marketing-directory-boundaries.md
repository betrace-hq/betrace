...
1. **Lacks integration with BeTrace's build system**: No `flake.nix`, no test integration
2. **Violates separation of concerns**: Marketing automation is external tooling, not core BeTrace functionality
3. **Creates maintenance burden**: Unmaintained code in main repository
4. **Confuses scope**: Unclear if this is a BeTrace feature or separate tool
...
- Simpler CI/CD for core BeTrace
...
- BeTrace core focuses on behavioral assurance
...
**Rejected**: Marketing automation is not a core BeTrace feature, shouldn't be in main repo regardless of build integration
