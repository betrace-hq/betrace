# FLUO AWS KMS Terraform Module - Outputs

# KMS Key Outputs

output "kms_key_id" {
  description = "KMS key ID (e.g., 'abcd1234-5678-90ab-cdef-1234567890ab')"
  value       = aws_kms_key.fluo_master_key.key_id
}

output "kms_key_arn" {
  description = "KMS key ARN (use this in FLUO backend configuration: aws.kms.master-key-id)"
  value       = aws_kms_key.fluo_master_key.arn
}

output "kms_key_alias" {
  description = "KMS key alias (e.g., 'alias/fluo-production')"
  value       = aws_kms_alias.fluo_master_key_alias.name
}

output "kms_key_alias_arn" {
  description = "KMS key alias ARN"
  value       = aws_kms_alias.fluo_master_key_alias.arn
}

# IAM Policy Outputs

output "iam_policy_arn" {
  description = "IAM policy ARN for FLUO KMS access"
  value       = aws_iam_policy.fluo_kms_access.arn
}

output "iam_policy_name" {
  description = "IAM policy name for FLUO KMS access"
  value       = aws_iam_policy.fluo_kms_access.name
}

# IAM Role Outputs (only if create_iam_role=true)

output "iam_role_arn" {
  description = "IAM role ARN for FLUO backend (only if create_iam_role=true)"
  value       = var.create_iam_role ? aws_iam_role.fluo_backend[0].arn : null
}

output "iam_role_name" {
  description = "IAM role name for FLUO backend (only if create_iam_role=true)"
  value       = var.create_iam_role ? aws_iam_role.fluo_backend[0].name : null
}

# Configuration Outputs (for FLUO backend setup)

output "fluo_backend_config" {
  description = "FLUO backend configuration snippet (copy to application.properties)"
  value = <<-EOT
    # FLUO Backend - AWS KMS Configuration
    # Generated by Terraform module: terraform/aws-kms

    fluo.kms.provider=aws
    aws.kms.master-key-id=${aws_kms_key.fluo_master_key.arn}
...
output "fluo_backend_env_vars" {
  description = "FLUO backend environment variables (for Docker/Kubernetes deployments)"
  value = {
    FLUO_KMS_PROVIDER          = "aws"
    AWS_KMS_MASTER_KEY_ID      = aws_kms_key.fluo_master_key.arn
...
output "validation_commands" {
  ...
  value = <<-EOT
    ...
    aws kms describe-key --key-id ${aws_kms_key.fluo_master_key.arn}
    aws kms get-key-rotation-status --key-id ${aws_kms_key.fluo_master_key.arn}

    # Test data key generation (simulates FLUO backend operation)
    aws kms generate-data-key \\
      --key-id ${aws_kms_key.fluo_master_key.arn} \\
      --key-spec AES_256 \\
      --encryption-context purpose=pii_redaction

    # Validate FLUO backend health check
...
output "security_checklist" {
  ...
  value = <<-EOT
    ✅ Security Checklist for FLUO AWS KMS Integration:

    1. Key Rotation
       - [ ] Verify automatic key rotation is enabled: ${aws_kms_key.fluo_master_key.enable_key_rotation}
