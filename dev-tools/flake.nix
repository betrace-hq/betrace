{
  description = "BeTrace Development Tools";
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Prompt Theme - ZSH theme for BeTrace
        betracePromptTheme = pkgs.writeTextFile {
          name = "betrace-prompt-theme.zsh";
          text = ''
            # BeTrace Prompt Theme with Test Stats
            # Auto-generated by BeTrace dev-tools
        ...
        setupPrompt = pkgs.writeShellScriptBin "setup-prompt" ''
          ...
          BETRACE_DEV_DIR="$HOME/.betrace-dev"
          mkdir -p "$BETRACE_DEV_DIR"

          # Copy prompt stats script
          cp ${promptStats}/bin/prompt-stats "$BETRACE_DEV_DIR/prompt-stats.sh"
          chmod +x "$BETRACE_DEV_DIR/prompt-stats.sh"

          # Copy ZSH theme
          cp ${betracePromptTheme} "$BETRACE_DEV_DIR/betrace-prompt-theme.zsh"

          echo "✅ BeTrace prompt theme installed to $BETRACE_DEV_DIR"
      ...
      in {
        packages = {
          ...
          prompt-theme = betracePromptTheme;
